...
class ButtonMaker:
    ...
    def button_link(self, key: str, link: str, position=None):
        match position:
            case 'header':
    ...
    def button_data(self, key: str, data: str, position=None):
        match position:
            case 'header':
    ...
    def button_menu(self, text: str, position=None):
        if not position:
            self._button.append(KeyboardButton(text))
        elif position == 'header':
            self._header_button.append(KeyboardButton(text))
        elif position == 'footer':
            self._footer_button.append(KeyboardButton(text))
    ...
    def build_menu(self, b_cols=1, h_cols=8, f_cols=8):
        ...
        return ReplyKeyboardMarkup(menu, resize_keyboard=True, selective=True, placeholder=self._placeholder) if self._placeholder else InlineKeyboardMarkup(menu)